...
def top_rotate(arr): ###공기청정기 위치 배제하기!!!
    #위쪽줄
    saved = arr[0][0]
    for x in range(C-1):
        arr[0][x]=arr[0][x+1]
    ...
    for y in range(purifier_i+1, R-1):
        arr[y][0] = arr[y+1][0]
    arr[R-2][0] = saved3
    arr[purifier_i+1][0] = -1 #공기청정기 위치이다.
...
for tc in range(1, trial+1):
    ...
    #room3 = [i for i in room] # 이것도 deepcopy 효과 낼 수 있당!!
