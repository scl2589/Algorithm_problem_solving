def f(i, j, c, e):
    ...
    for m in range(4):
        ni = i + di[m]
        nj = j + dj[m]
        if 0<=ni <N and 0<=nj <N: #유효 좌표인지 확인
            if arr[i][j] > arr[ni][nj]:
                f(ni, nj, c, e+1) #주변의 낮아진 칸으로 이동
        #깎아서 더 낮아지는 경우, 등산로에 포함되지 않은 칸이면
        elif visited[ni][nj] == 0 and c>0 and arr[i][j] > arr[ni][nj]- K:
            org = arr[ni][nj] #원래 높이 저장
            arr[ni][nj] = arr[i][j] -1 #주변칸을 깎아서 이동
            f(ni, nj, 0, e+1)
...
for tc in range(1, T+1):
    ...
    for i in range(N):
        for j in range(N):
            if arr[i][j] == h:
                f(i, j, 1, 1) #좌표i, j, 남은 깎음 횟수, 현재 조성된 등산로 길이
